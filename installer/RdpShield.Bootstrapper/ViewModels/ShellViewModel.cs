...
namespace Bootstrapper.ViewModels
{
  internal class ShellViewModel : ViewModelBase
  {
    ...
    public string Branding => "RDP Shield";
    public bool ShowLogLink => _model.State.BaStatus == BaStatus.Failed || _model.State.BaStatus == BaStatus.Cancelled || _model.State.BaStatus == BaStatus.Applied;
    ...
    public void AfterDetect(LaunchAction followupAction)
    {
      ...
      finally
      {
        ...
        OnPropertyChanged(nameof(ShowLogLink));
      }
    }
    ...
    public void AfterApply()
    {
      try
      {
        if (_model.State.BaStatus == BaStatus.Applied)
          ProgressVm.Complete();
        else
          ProgressVm.Reset();
        AssignMessage();
